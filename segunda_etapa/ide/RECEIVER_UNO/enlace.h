...
const uint16_t PAYLOAD_MUESTRA_MAX = Protocolo::MAX_CARGA_UTIL_PROYECTO;
...
struct TramaLigera {
  ...
  uint16_t longitud;
  uint16_t crc;
  const uint8_t *payloadMuestra;
  uint16_t payloadMuestraLongitud;
};
...
private:
  ...
  uint8_t crcAlto_;
  uint8_t payloadBuffer_[PAYLOAD_MUESTRA_MAX];
};
